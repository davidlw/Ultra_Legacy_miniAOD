...
//std::cout<<CN2_17_<<" "<<CN4_51_<<std::endl;
  ...
  for(int i=0;i<hcN2->GetNbinsX();i++)
  {
    ...
    double cum4 = sumCN4_51_[i] - 2*sumCN2_17_[i]*sumCN2_17_[i];
    ...
    hcN2->SetBinContent(i+1,cum2);
    hcN4->SetBinContent(i+1,cum4);
    hcN6->SetBinContent(i+1,cum6);
    hcN8->SetBinContent(i+1,cum8);
  }

  TFile * outputFile = TFile::Open("output_1.root","recreate");
  hcN2->Write();
  hcN4->Write();
  hcN6->Write();
  hcN8->Write();
